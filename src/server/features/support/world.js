  ...
  scriptByWebsite: () => {
    ...
    return {
      ...
      SmsGagnant: {
        scriptListUrlEvents: (selectors) => `
        const elems = Array.from(document.querySelectorAll("${selectors}"))
        ${this.returnString} elems.map((elem) => elem.getAttribute('href')).filter((value, index, self) => value !== null &&  self.indexOf(value) === index && value.indexOf('/Promo/') !== -1 );
      `,
        scriptGetTitle: `
          ${this.returnString} document.querySelector(".shipping-detail > strong").textContent.trim() ?? ""`,
        scriptGetDate: `
              const durationString = document.querySelector(".compte_a_rebours").textContent.trim();

              const jours = durationString.match(/(\\d+) (jours|jour)/)?.[1] ?? 1;
              const [, heures, minutes, secondes] = durationString.match(/(\\d+):(\\d+):(\\d+)/);
